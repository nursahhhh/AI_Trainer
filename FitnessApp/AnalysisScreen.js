  ...
  const stopAnalysis = async (stoppedByLimit = false) => {
    try {
      ...
      const payload = buildAnalysisPayload({
        exerciseType,
        elapsedSeconds,
        frames: keypointBufferRef.current.slice(0, 3), // İlk 3 frame'i gönderiyoruz, gerçek durumda tümünü göndereceğiz
      });
  ...
  const handleDetailedAnalysis = async () => {
    ...
    try {
     ...
     const payload = buildDetailedAnalysisPayload({
      ...
      const feedbackResult = await generateFeedbackReport(predictionResult);
setDetailedAnalysis(feedbackResult.report);;
